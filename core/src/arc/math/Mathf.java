...
public final class Mathf{
    ...
    public static int select(int v1, int v2){
        return rand.nextBoolean() ? v1 : v2;
    }

    public static float select(float v1, float v2){
        return rand.nextBoolean() ? v1 : v2;
    }
    ...
    public static float sqrwavex(float angle){
        angle %= 360f;
        if(angle < 0) angle += 360f;
        if(angle < 45 || angle >= 315) return 1f;
        if(angle >= 45 && angle < 135) return 1f - (angle - 45f) / 45f;
        if(angle >= 135 && angle < 225) return -1f;
        return -1f + (angle - 225f) / 45f;
    }

    public static float sqrwavey(float angle){
        angle %= 360f;
        if(angle < 0) angle += 360f;
        if(angle >= 45 && angle < 135) return 1f;
        if(angle >= 135 && angle < 225) return 1f - (angle - 135f) / 45f;
        if(angle >= 225 && angle < 315) return -1f;
        if(angle < 45) return angle / 45f;
        return -1f + (angle - 315f) / 45f;
    }

    public static void traverseSpiral(int w, int h, int radius, Boolf2<Integer, Integer> cons){
        int x = 0, y = 0, dx = 0, dy = -1;
        int max = radius * radius * 4;
        for(int i = 0; i < max; i++){
            if((-radius <= x) && (x <= radius) && (-radius <= y) && (y <= radius)){
                if(cons.get(x + w / 2, y + h / 2)) return;
            }
    ...
    public static boolean in(float value, float dest, float range){
        return Math.abs(dest - value) < range;
    }
    ...
    /** Linearly interpolates between fromValue to toValue on progress position. Exponentially based on Time.delta.*/
    public static float lerpDelta(float fromValue, float toValue, float progress){
        return lerp(fromValue, toValue, 1f - pow(1f - progress, Time.delta));
    }
    ...
    public static float slerpDelta(float fromDegrees, float toDegrees, float progress){
        return slerp(fromDegrees, toDegrees, 1f - pow(1f - progress, Time.delta));
    }
     ...
     */
    public static int scl(float value, int scl){
        return (int)(value / scl);
    }
    ...
    public static int scl2(float value, float scl){
        return (int)(value / scl);
    }
