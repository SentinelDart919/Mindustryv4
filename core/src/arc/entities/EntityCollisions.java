...
public class EntityCollisions{
    ...
    private Seq<SolidTrait> arrOut = new Seq<>();
            ...
            for(int dy = -r; dy <= r; dy++){
                ...
                if(collider.solid(wx, wy)){
                    ...
                    if(tmp.overlaps(rect)){
                        Vec2 v = Physics.overlap(rect, tmp, x);
                        rect.x += v.x;
                        rect.y += v.y;
                    }
                }
    ...
    public void collideGroups(EntityGroup<?> groupa, EntityGroup<?> groupb){
        ...
        for(Entity entity : groupa.all()){
            if(!(entity instanceof SolidTrait) || collided.contains(entity.getID())) continue;
    ...
    public interface TileHitboxProvider{
        void getHitbox(int x, int y, Rect out);
    }

    public interface BoundingBoxProvider<T>{
        void getBoundingBox(T type, Rect out);
    }
}